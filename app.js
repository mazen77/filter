...
function portal(lms,box,w,h,now){
 ...
 ctx.save();polyPath(p);ctx.clip();ctx.filter=worlds[world].filter;
 if(worlds[world].pixel){off.width=Math.max(24,Math.floor(w/18));off.height=Math.max(40,Math.floor(h/18));ox.save();ox.translate(off.width,0);ox.scale(-1,1);ox.drawImage(video,0,0,off.width,off.height);ox.restore();ctx.imageSmoothingEnabled=false;ctx.drawImage(off,0,0,w,h);ctx.imageSmoothingEnabled=true}
 else if(worlds[world].mirror){ctx.save();ctx.translate(w,0);ctx.scale(-1,1);ctx.drawImage(canvas,0,0,w,h);ctx.restore()}
 else drawVideo(w,h);ctx.restore();ctx.filter="none";
...
$("#startBtn").onclick=init;$("#retryBtn").onclick=init;$("#recordBtn").onclick=toggleRecord;$("#saveBtn").onclick=snapshot;$("#againBtn").onclick=()=>show(ui.result,false);
$("#flipBtn").onclick=async()=>{facing=facing==="user"?"environment":"user";await startCamera()};
$("#soundBtn").onclick=()=>tone(600,.12);
